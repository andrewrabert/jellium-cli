...
pub fn view<'a>(state: &'a State, read_only: bool) -> Element<'a, Message> {
    let mut page = column![text(strings::lookup(Text::KeysTitle)).size(22)]
    ...
    if !read_only {
        page = page.push(
            row![
                ...
                button(text(strings::lookup(Text::KeysCreate))).on_press(Message::DashboardAction(
                    super::Action::Write(super::Written::CreateKey {
    ...
    for key in &state.keys {
        ...
        .spacing(theme::CARD_SPACING);
        if !read_only {
            held = held.push(button(text(strings::lookup(Text::KeysRevoke))).on_press(
