...
pub fn view<'a>(state: &'a State, read_only: bool) -> Element<'a, Message> {
    let mut page = column![text(strings::lookup(Text::RepositoriesTitle)).size(22)]
    ...
    for repository in &state.repositories {
        ...
        let mut held = row![
            text(repository.name.clone().unwrap_or_default()),
            text(url.clone()),
        ]
        .spacing(theme::CARD_SPACING);
        if !read_only {
            held = held.push(
                button(text(strings::lookup(Text::RepositoriesRemove))).on_press(
    ...
    if !read_only {
        page = page.push(
            row![
                ...
                button(text(strings::lookup(Text::RepositoriesAdd))).on_press(
                    Message::DashboardAction(super::Action::Ask(
                        crate::screen::confirm::Pending::of(
